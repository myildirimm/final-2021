...
class Client():
    ...
    def __init__(self,host="127.0.0.1",port=8080,timeout=10):
        ...
        self.client = msgpackrpc.Client(msgpackrpc.Address(host, port), timeout = timeout, \
        ...
        try:
            ...
            if server_info == "":
                ...
                self.client = msgpackrpc.Client(msgpackrpc.Address(self.server_ip, self.server_port), timeout = timeout, \
                    pack_encoding = 'utf-8', unpack_encoding = 'utf-8')
        except TransportError:
            raise TransportError(
            "\n Make sure SimStar is running from port: %d at: %s"%(int(port),host))
    ...
    def reconnect_clients(self):
        ...
        while(time.time()-start_time<self.timeout):
            try:
                self.local_client = msgpackrpc.Client(msgpackrpc.Address(self.host, self.port),
                                                timeout=self.timeout,
                                                pack_encoding='utf-8',
                                                unpack_encoding='utf-8')
                self.client = msgpackrpc.Client(msgpackrpc.Address(self.server_ip, self.server_port),
                                                timeout=self.timeout,
                                                pack_encoding='utf-8',
                                                unpack_encoding='utf-8')
                self.ping()
    ...
    def adjust_view(self, x_in_m, y_in_m, z_in_m, 
        ...
        self.client.call("AdjustActiveView", x_in_m, y_in_m, z_in_m,
                        roll, pitch, yaw, fov)
        
    def spawn_vehicle(self, actor=0, distance=20, lateral_offset=0, lane_id=1, initial_speed=30, 
                      set_speed=50, trajectory_data=[], vehicle_type = EVehicleType.Sedan1,
                      vehicle_model=EVehicleModel.PhysX):
        if actor == 0:
            if(lateral_offset != 0):
                vehicle_id = self.client.call("SpawnVehicleTo", distance, lateral_offset, lane_id, 
                                            initial_speed, set_speed, trajectory_data, 
                                            vehicle_type, vehicle_model)
            else:
                vehicle_id = self.client.call("SpawnVehicle", distance, lane_id, 
                                            initial_speed, set_speed, trajectory_data, 
                                            vehicle_type, vehicle_model)
        else:
            if(lateral_offset != 0):
                vehicle_id = self.client.call("SpawnVehicleRelativeToActor", actor.get_ID(), distance, 
                                              lateral_offset, lane_id, initial_speed, set_speed, 
                                              trajectory_data, vehicle_type,vehicle_model)
            else:
                vehicle_id = self.client.call("SpawnVehicleRelativeTo", actor.get_ID(), distance, 
                                              lane_id, initial_speed, set_speed, 
                                              trajectory_data, vehicle_type,vehicle_model)
        return Vehicle(self.local_client,vehicle_id)

    def spawn_merging_vehicle(self, actor = 0, distance = 0, 
                              initial_speed = 30, set_speed = 50,
                              trajectory_data = [],
        ...
        vehicle_id = self.client.call("SpawnMergingVehicle", actor_id, distance, 
                                      initial_speed, set_speed, trajectory_data, 
                                      vehicle_type, vehicle_model)
        return Vehicle(self.local_client,vehicle_id)

    def spawn_vehicle_to(self, vehicle_pose,
                         initial_speed = 50, set_speed = 50,
                         trajectory_data = [],
                         vehicle_type = EVehicleType.Sedan1,
                         vehicle_model = EVehicleModel.PhysX):
        vehicle_id = self.client.call("SpawnVehicleByLocation", vehicle_pose,
                                      initial_speed, set_speed,
                                      trajectory_data, vehicle_type, vehicle_model)
        return Vehicle(self.local_client,vehicle_id)
    ...
    def check_collisions(self):
        if (not self.collision_client):
            ...
            self.collision_client = msgpackrpc.Client(msgpackrpc.Address(host, port), timeout = timeout, \
                pack_encoding = 'utf-8', unpack_encoding = 'utf-8')
    ...
    def spawn_pedestrian(self,actor=0,distance=20,from_right_to_left=True):
        pedestrian_id = -1
        if actor != 0:
            pedestrian_id = self.client.call("SpawnPedestrianRelativeTo", actor.get_ID(), distance, from_right_to_left)
        return Pedestrian(self.client,pedestrian_id)

    def spawn_random_walker(self,
                            walking_speed=5,
                            pedestrian_type=EPedestrianType.Female1):
        pedestrian_id = self.client.call("SpawnRandomWalker",
                                         walking_speed,
                                         pedestrian_type)
        return Pedestrian(self.client, pedestrian_id)
    ...
    def spawn_pedestrian_to_location(self,
                                     location,
                                     target_location,
                                     walking_speed=5,
                                     pedestrian_type=EPedestrianType.Female1):
        pedestrian_id = self.client.call("SpawnPedestrianToLocation",
                                         location,
                                         walking_speed,
                                         target_location,
                                         pedestrian_type)
    ...
    def spawn_pedestrian_relative_to_location(self,
                                              origin,
                                              relative_spawn_location,
                                              target_location,
                                              walking_speed=5,
                                              pedestrian_type=EPedestrianType.Female1):
        pedestrian_id = self.client.call("SpawnPedestrianRelativeToLocation",
                                         origin,
                                         relative_spawn_location,
                                         walking_speed,
                                         target_location,
    ...
    def create_world_from_xodr(self,xodr_content,add_mesh=True,add_debug=False):
        #TODO: should we change to Hihgway Map before this?
        self.client.call("CreateFromOpenDrive",
            xodr_content,add_mesh,add_debug)
    ...
    def compare_simulator_version_with_api(self):
        try:
            ...
            if sim_version != api_version:
                print(bcolors.WARNING + "[simstar] [Warning] Version Mismatch" + bcolors.ENDC)
    ...
    def create_infinite_highway(self,is_curved, min_radius,
         is_traffic_enabled,time_interval=5,vehicle_model=0):
        is_created = self.client.call("InfiniteHighway", is_curved, min_radius, 
            is_traffic_enabled,time_interval,vehicle_model)
        return is_created
    ...
    def get_tick_number(self):
        return self.client.call("GetPassedTickNumber")
    ...
    def get_waypoints(self, vehicle, lane_id = 1, starting_offset = 0, offset = 0, count = 20, interval = 10):
        return self.client.call("GetWayPoints", vehicle.get_ID(), lane_id, starting_offset, offset, count, interval)
    ...
    def spawn_road_work(self,actor=0,distance=0,lane=1,length=10):
        road_work_id = self.client.call("SpawnRoadWorkRelativeTo", actor.get_ID(), distance, lane, length)
        return RoadWork(self.client,road_work_id)

    def spawn_speed_limit(self,actor=0,distance=0,speed_limit=ESpeedLimitState().SpeedLimit_30):
        speed_limit_id = self.client.call("SpawnSpeedLimitRelativeTo", actor.get_ID(), distance, speed_limit)
        return SpeedLimit(self.client,speed_limit_id)

    def spawn_traffic_sign_relatively(self,actor=0,distance=0,sign='AcilanKopru'):
        sign_id = self.client.call("SpawnTrafficSignRelativeTo", actor.get_ID(), distance, sign)
    ...
    def set_weather(self,rain=0,snow=0,fog=0,dust=0,delay=0.0):
        if(delay==0.0):
            ...
            return self.client.call("UpdateWeatherAfterTime", 
                                    rain,snow,fog,dust,delay) 
    def set_weather_after_distance(self,actor,
        rain=0,snow=0,fog=0,dust=0,distance=10.0):
        actor_id = actor.get_ID()
        return self.client.call("UpdateWeatherAfterDistance", 
                                    actor_id,rain,snow,fog,dust,distance)                                 
    ...
    def save_scenario(self, scenario_name,scenario_description=""):
    ...
    def get_pose_of_point_along_road_spline(self, road_generator, lane_id, distance):
        ...
        return self.client.call("GetPoseOfPointAlongRoadSpline", road_generator_id, lane_id, distance)
    ...
    def update_spot_info(self, spot_name, grid_width=3, grid_height=6, lane_thickness=0.3):
        return self.client.call("UpdateSpotInfo", spot_name, grid_width, grid_height, lane_thickness)
    ...
    def make_spot_unavailable(self, spot_name, offset_x=0, offset_y=0, yaw_in_degree=0):
        return self.client.call("MakeSpotUnavailable", spot_name, offset_x, offset_y, yaw_in_degree)
